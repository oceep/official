    ...
    chatFormEl.addEventListener('submit', async function(event) {
        ...
        if (initialView && !initialView.classList.contains('hidden')) {
            ...
            setTimeout(() => {
                ...
                if(mainContent) { mainContent.classList.remove('justify-center'); mainContent.classList.add('justify-start'); }
            }, 500);
        }
        ...
        try {
            ...
            const fullAiResponse = await streamAIResponse(modelToUse, conversationHistory, aiEl, abortController.signal);
            conversationHistory.push({ role: 'assistant', content: fullAiResponse });
            saveStateToLocalStorage(); 
        } catch (error) {
            // Error handled in streamAIResponse
        } finally {
