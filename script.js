...
// Hàm này được gọi từ các nút "Copy" trong các block code
...
// Cấu hình Hệ thống Token (Giới hạn lượt chat)
const tokenConfig = {
    IS_INFINITE: true,           // Nếu true: Chat thoải mái không giới hạn
    MAX_TOKENS: 50,              // Số token tối đa
    TOKEN_COST_PER_MESSAGE: 1,   // Phí cho mỗi tin nhắn
    TOKEN_REGEN_INTERVAL_MINUTES: 5, // Thời gian hồi phục
    TOKEN_REGEN_AMOUNT: 1,       // Số lượng hồi phục
};
...
let abortController; // Để hủy request đang chạy
let isRandomPromptUsedInSession = false;
let conversationHistory = []; // Hội thoại hiện tại
let chatHistories = {}; // Lưu tất cả hội thoại: { timestamp: [messages] }
...
const translations = {
    vi: {
        ...
        outOfTokensPlaceholder: "Bạn đã hết lượt.", 
        sendButton: "Gửi", 
        stopButton: "Dừng", 
        ...
        learnButton: "Học Tập", 
        footerText: "AI có thể mắc lỗi. Hãy kiểm tra thông tin quan trọng.", 
        ...
        modelMiniDesc: "Nhanh và hiệu quả.", 
        modelSmartDesc: "Cân bằng tốc độ và thông minh.", 
        modelNerdDesc: "Suy luận cao, kết quả chuẩn xác."
    },
    en: {
        ...
        outOfTokensPlaceholder: "You're out of tokens.", 
        sendButton: "Send", 
        stopButton: "Stop", 
        modelButtonDefault: "Expert", 
        ...
        learnButton: "Study", 
        footerText: "AI can make mistakes. Check important info.", 
        ...
        comingSoonText: "Under development.", 
        langTooltip: "Switch Language", 
        themeTooltip: "Change Theme", 
        historyTooltip: "Chat History", 
        newChatTooltip: "New Chat", 
        modelMiniDesc: "Fast and efficient.", 
        modelSmartDesc: "Balanced speed and intelligence.", 
        modelNerdDesc: "Powerful model for complex answers."
    },
};
// Fallback cho các ngôn ngữ khác về Tiếng Anh
...
// Logic phân loại Search (để hiển thị UI "Đang tìm kiếm...")
...
// FORMATTER CHÍNH (Đã tích hợp Source Pill & Markdown Table)
...
// API STREAMING (CÓ TIMEOUT 60S & XỬ LÝ LỖI MẠNH MẼ)
async function streamAIResponse(modelName, messages, aiMessageEl, signal) {
    ...
    try {
        // Tự tạo timeout phía Client (60 giây)
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 60000);
        
        // Kết hợp signal hủy từ nút Stop và signal timeout
        ...
        if (error.name === 'AbortError') {
            // Phân biệt do người dùng bấm Stop hay do Timeout
            if (!signal?.aborted) {
                // Đây là do Timeout 60s
